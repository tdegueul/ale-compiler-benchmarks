...
public class ControlFlowImpl extends ActivityEdgeImpl implements ControlFlow {
	...
	protected EClass eStaticClass() {
		return AdmodularPackage.Literals.CONTROL_FLOW;
	}
		...
		if (guard != null && guard.eIsProxy()) {
			...
			if (guard != oldGuard) {
				if (eNotificationRequired())
					eNotify(new ENotificationImpl(this, Notification.RESOLVE, AdmodularPackage.CONTROL_FLOW__GUARD, oldGuard, guard));
			}
		}
		return guard;
	}
	...
	public void setGuard(BooleanVariable newGuard) {
		...
		guard = newGuard;
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, AdmodularPackage.CONTROL_FLOW__GUARD, oldGuard, guard));
	}
	...
	public Object eGet(int featureID, boolean resolve, boolean coreType) {
		switch (featureID) {
			case AdmodularPackage.CONTROL_FLOW__GUARD:
